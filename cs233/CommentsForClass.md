...
- This is one shared staff machine rather than per-student VMs, so there is no memory saving to chase; the only reason to containerise it is consistency with everything else.
- **Blocker:** the course's own SPIM fork (`spim-vasm`), which every lab Makefile calls, has no public source or package. We cannot build a working image without it.
- **To confirm:** what the machine is actually used for. We believe it is a tournament runner or staff build box, and that the autograder runs on hosted PrairieLearn rather than here.

A draft, never-built Dockerfile for this class is in `Dockerfile.suggested` in this directory. Because of the `spim-vasm` blocker it cannot be completed without your help.
