...
- Better suited to containers than we expected, because students already boot their own kernel in QEMU rather than loading modules into the machine's kernel. The container would hold the build toolchain and QEMU.
- **The catch is hardware acceleration.** QEMU needs access to the host's KVM device to run at full speed; without it, emulation may be several times slower, which could distort the timing measurements in MP2 and MP3. Granting that access is a security decision for the host owners.
- **To confirm:** whether KVM access is acceptable, and whether slower emulation would affect grading.
