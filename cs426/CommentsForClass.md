...
- Straightforward fit: SSH or VS Code, no root needed, no special privileges. The image would ship LLVM 19 and the course toolchain.
- **The memory question is MP4**, where students patch and rebuild LLVM themselves. That build is the only reason the course needs a large machine. If the image shipped a pre-patched, pre-built LLVM, rebuilds would be far cheaper — but the handout would need changing, so this is your call.
- **To confirm:** how the handin script reaches the network, and whether a prebuilt LLVM is acceptable pedagogically.
