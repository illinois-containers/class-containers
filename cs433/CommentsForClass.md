...
- The cleanest fit of any class we looked at: everything is ordinary user-space simulation, with no special privileges and no kernel features.
- **Two easy wins:** shipping gem5 already built in the image (avoiding a long build per student), and putting the shared traces and resources on one read-only volume instead of a copy per student.
- **Caveat:** the lab handouts are password-protected, so our list of dependencies is a guess. A copy of the setup script would let us verify it.
