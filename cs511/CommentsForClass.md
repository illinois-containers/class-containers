...
- Feasible, with one requirement: two of the three MPs have students run Docker themselves, so the platform must support Docker inside the container (safely — not by handing out host access).
- **The disk request looks larger than the MPs need**: the benchmark data is about a gigabyte, and since it is generated deterministically, staff could produce it once as a shared read-only volume.
- **Note:** the MP autograders already run on GitHub-hosted runners and grade correctness rather than speed, so container timing differences do not affect MP grades — only the group project's measurements.
